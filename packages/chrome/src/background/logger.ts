/**
 * Structured logger for the Veil extension service worker.
...
function log(
  ...
  // Format: [LEVEL] [namespace] message { context }
...
export async function flushLogsToStorage(): Promise<void> {
  ...
  try {
    const existing = (await chrome.storage.local.get("logs")).logs as LogEntry[] | undefined;
    const combined = (existing || []).concat(logBuffer).slice(-MAX_BUFFER_SIZE);
    await chrome.storage.local.set({ logs: combined });
    logBuffer.length = 0;
  } catch {
